...
## 4 日志输出到文件
    ...
    log4j给我们提供了一种比较使用的文件日志框架：可以按时间戳生成日志文件。
    配置如下：
```properties
# 设置
...
#### 4.1 输出的日志文件
    ...
    例如，这里指定的是DatePattern=.yyyy-MM-dd。则昨天的日志文件名称就是ai88.log.2020-10-03
...
#### 5.1 等级关系
    日志等级按照严重关系为debug<info<warn<error<fatal<off。一般，我们只用到前面四个。
